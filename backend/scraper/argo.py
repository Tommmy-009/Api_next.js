...
def _parse_teachers_table(page: Page) -> list[dict]:
    ...
    for table in page.locator("table, div.btl-table").all():
        ...
        teacher_index = next(
            (i for i, value in enumerate(headers) if any(word in value for word in ("docente", "professore", "insegnante"))),
            None,
        )
        subject_index = next(
            (i for i, value in enumerate(headers) if any(word in value for word in ("materia", "disciplina", "insegnamento"))),
            None,
        )
